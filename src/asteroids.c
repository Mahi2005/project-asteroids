...
#define DEBUG 0
extern int buffer;
extern float ship_invuln_time;
extern bool ship_invuln_flag;
...
void Asteroid_track_count(Asteroid_T asteroids[]) {
    ...
    for (int i = 0; i < max_asteroids; i++) {
        if (asteroids[i].state > 0)
            asteroids_count++;
    }
}

void Asteroid_copy(Asteroid_T *a1, Asteroid_T *a2) {
    // copies the position, radius, vertices, bitcodes and velocity of a2 into
    // a1
...
void Asteroid_shape_generate(Asteroid_T *a) {
    ...
    for (int i = 1; i <= pos_y_n_max; i++) {
        ...
        float y = a->position.y +
                  sqrtf(powf(a->radius, 2) - powf(x - a->position.x, 2));
    ...
    for (int i = 0; i < 4; i++) {
        ...
        a->vertices[n] = Vector2ScaleRelative(
            a->vertices[n], get_random_float(0.5, 1), a->position);
    }
}
...
void Asteroid_rand_init(Asteroid_T *a) {
    ...
    init_positions[0] =
        Vector2Create(GetRandomValue(-buffer / 2, 0), GetRandomValue(0, sc_h));
    init_positions[1] =
        Vector2Create(GetRandomValue(0, sc_w), GetRandomValue(-buffer / 2, 0));
    init_positions[2] = Vector2Create(GetRandomValue(sc_w, sc_w + buffer / 2),
                                      GetRandomValue(0, sc_h));
    init_positions[3] = Vector2Create(GetRandomValue(0, sc_w),
                                      GetRandomValue(sc_h, sc_h + buffer / 2));
    ...
    a->velocity =
        Vector2Rotate(Vector2Scale(Vector2Create(0, -1), speed), rotation);
...
void Asteroid_reposition(Asteroid_T *a, Vector2 displacement) {
    ...
    // for () {
    //    a->vertices[i] = Vector2RotateAround(a->vertices[i], a->rotation,
    //    a->position);
    // }
}
...
void Asteroid_rescale(Asteroid_T *a, float scale) {
    for (int i = 0; i < NVERTICES; i++) {
        a->vertices[i] =
            Vector2ScaleRelative(a->vertices[i], scale, a->position);
    }
    a->radius *= scale;
}
...
void Asteroid_draw(Asteroid_T *a, Color color) {
    ...
    if (DEBUG) {
        for (int i = 0; i < NVERTICES; i++) {
            ...
            int code = vertex_code(a->vertices[i], GetScreenWidth(),
                                   GetScreenHeight());
            sprintf(debug, "Point %d, %s", i,
                    ((code > 0) && (code <= 15)) ? "crossed" : "uncrossed");
...
void Asteroid_fragment_or_destruct(Asteroid_T asteroids[], Asteroid_T *a) {
    ...
    case 3:
        ...
        medium_asteroid->velocity =
            Vector2Scale(Vector2Rotate(medium_asteroid->velocity, rand_angle),
                         rand_velocity_factor);
        ...
        a->velocity = Vector2Scale(Vector2Rotate(a->velocity, rand_angle),
                                   rand_velocity_factor);
        Asteroid_rescale(a, 0.5);
    ...
    case 2:
        ...
        small_asteroid->velocity =
            Vector2Scale(Vector2Rotate(small_asteroid->velocity, rand_angle),
                         rand_velocity_factor);
...
void Asteroid_strike_ship(Asteroid_T asteroids[], Ship_T *ship) {
    for (int i = 0; i < max_asteroids; i++) {
        ...
        bool collision =
            CheckCollisionCircles(asteroids[i].position, asteroids[i].radius,
                                  ship->centroid, ship_radius);
        if (((asteroids[i].state && ship->intact) && ship_invuln_time <= 0) &&
            collision) {
            ship->intact = 0;
            // asteroids[i].state = 0;
            lives--;
